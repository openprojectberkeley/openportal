-- Returning members now bypass BOTH the coffee-chat and info-session
-- requirements, not just the coffee chat.
--
-- Previous predicate (0067): board_or_exec OR ((completed OR returning) AND attended)
-- New predicate:             board_or_exec OR returning OR (completed AND attended)
--
-- Recreates both analytics functions from 0067 with the new predicate. Per-funnel
-- coffee/info category counts (coffee_completed/booked/returning/nothing,
-- info_attended/returning/nothing) are unchanged — they still report actual
-- attendance; only the combined "valid" measure changes.
--
-- Signatures are unchanged from 0067, so `create or replace` suffices.
...
begin
  ...
  select
    ...
    count(*) filter (where f.sub and (f.is_be or f.is_ret or (f.did_complete and f.did_att)))::int
...
-- Per-person "invalid" list — returning members now excluded (auto-valid),
-- same as board/exec.
