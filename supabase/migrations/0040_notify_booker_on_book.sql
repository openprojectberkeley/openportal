-- Also notify the BOOKER (applicant) when they book a coffee chat, so they get
-- the confirmation email + calendar invite too — not just the host.
--
-- notify_coffee_chat_counterparty (0026 → 0038 copy → 0039 timezone) notifies a
-- single counterparty: on 'chat_booked' the caller is the applicant and the
-- recipient is the host. The host gets an in-app notice, an email, and (via
-- send-notification-email) an ICS invite; the applicant who did the booking got
-- nothing. This adds a second 'chat_booked' notification addressed to the
-- applicant, with booker-facing copy localized to the APPLICANT's own timezone.
--
-- Why no email/edge-function change is needed: the extra row is still type
-- 'chat_booked', which the 0037 email trigger already forwards, and the edge
-- function already builds an ICS REQUEST for that type with ATTENDEE = the row's
-- owner (here, the booker) and UID = the coffee_chat_id (same event, per-attendee
-- invite — exactly how calendar invites work).
--
-- Same signature as 0039, so create-or-replace works in place. Only change: the
-- 'chat_booked' branch also inserts a booker-facing row for v_applicant.
