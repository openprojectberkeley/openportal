-- Undo 0081/0084: late submission no longer makes an applicant recruiting-invalid.
-- Keep shared is_recruiting_valid so both_valid and the invalid list stay in sync.
-- Late badge (client isLate/daysLate) is unchanged — only the validity rule reverts.
-- Formula: board/exec OR returning OR (coffee complete AND info attended).
...
as $$
  -- p_submitted_at / p_ends_at kept for call-site compatibility; unused after
  -- removing the 3+ day late validity rule.
  select
    exists (
      select 1
...
-- both_valid already goes through is_recruiting_valid; recreate so deploy of this
-- file alone refreshes the body that calls the helper (no signature change).
...
-- Drop is_severely_late from the invalid list return type.
